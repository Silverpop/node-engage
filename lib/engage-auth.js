...
var EngageAuth = function(engage, hostname, transactHostname, clientId, clientSecret, refreshToken) {
    ...
    var _cache = null;
    var _engage = engage;
    var _options = engage.getOptions();
    var _hostname = hostname;
    var _transactHostname = transactHostname;
    ...
    if (_options.cacheAccessTokens === true && _options.cacheFile !== undefined) {
        _cache = require('node-file-cache').create({file: _options.cacheFile, life: 3600});
    }
    ...
    function _getTransactApiUrl(path) {
        ...
        return 'https://' + _transactHostname + path;
    ...
    function _getOAuthAccessToken(cb) {
        ...
        if (_cache !== null) {
            var _cachedToken = _cache.get(_clientId + '::token');
            var _cachedExpirationTime = _cache.get(_clientId + "::expiration")
            if (_cachedToken !== null) {
                _oAuthAccessToken = _cachedToken;
                _expirationTime = _cachedExpirationTime;
            }
            if (_engage.getDebugLevel() >= 2) {
                console.log("Cached token:  " + _cachedToken);
                console.log("Cached token expiration:  " + _cachedExpirationTime);
            }
        }
    ...
    my.makeRequest = function(path, method, type, postData, cb) {
        ...
        _getOAuthAccessToken(function(err, token) {
            ...
            if (_cache !== null) {
                ...
                _cache.set(_clientId + '::token', token);
                _cache.set(_clientId + '::expiration', _expirationTime);
            }
            ...
            if (path.indexOf('XTMail') !== -1) {
                options.hostname = _transactHostname;
                if (_engage.getDebugLevel >= 2) {
                    console.log("Transact API Request to '" + options.hostname + '"');
                }
            }
